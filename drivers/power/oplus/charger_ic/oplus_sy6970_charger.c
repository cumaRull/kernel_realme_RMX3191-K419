...
struct bq2589x {
	...
	struct delayed_work bq2589x_enable_interrupt_work;
...
static int bq2589x_register_interrupt(struct device_node *np,struct bq2589x *bq)
{
	...
	schedule_delayed_work(&g_bq->bq2589x_enable_interrupt_work, msecs_to_jiffies(5000));
...
void oplus_bq2589x_batt_temp_status_check(void)
{
	...
	dev_info(g_bq->dev, "%s:battery temp %d,batt_temp_flag %d\n",  __func__, g_oplus_chip->tbatt_temp, g_bq->batt_temp_flag);
	if ((g_oplus_chip->charger_volt > 7500) && (g_oplus_chip->tbatt_temp > BQ_HOT_TEMP_TO_5V)) {
		if (g_bq->hvdcp_can_enabled == true) {
			...
			g_bq->hvdcp_can_enabled = false;
			g_bq->batt_temp_flag = true;
			dev_info(g_bq->dev, "%s:battery temp is high, disable hvdcp\n",  __func__);
		}
	} else if (g_oplus_chip->tbatt_temp < BQ_HOT_TEMP_TO_5V && g_bq->batt_temp_flag) {
		if ((g_oplus_chip->ui_soc < 90) || (g_oplus_chip->batt_volt < 4250)) {
			if (!(g_bq->is_bq2589x)) {
				bq2589x_enable_hvdcp(g_bq);
				if (is_project(0x216AF) || is_project(0x216B0) || is_project(0x216B1)){
...
int oplus_bq2589x_get_charger_subtype(void)
{
	...
	} else if (g_bq->hvdcp_can_enabled || g_bq->batt_temp_flag) {
...
int oplus_bq2589x_set_qc_config(void)
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > 7500
		...
		dev_info(g_bq->dev, "%s: set qc to 5V", __func__);
			...
			if (g_bq->batt_temp_flag) {
				dev_info(g_bq->dev, "%s: Hihg temperature do not enter QC 9v charging", __func__);
				return false;
			}
...
static void charging_enable_interrupt_work(struct work_struct *work)
{
...
//mofify for hardware
	...
	INIT_DELAYED_WORK(&bq->bq2589x_enable_interrupt_work, charging_enable_interrupt_work);
