...
#include "../vendor/oplus/kernel/mm/memleak_detect/memleak_debug_stackdepot.h"
...
ssize_t hash_val_read(struct file *file, char __user *buf, size_t count, loff_t *ppos) {
    return 0; // Dummy read implementation
	}

ssize_t hash_val_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos) {
	return count; // Dummy write implementation
	}
...
#else
	static const struct file_operations hash_val_ops = {
	.owner = THIS_MODULE,
	.read = hash_val_read,
	.write = hash_val_write,
	};
#endif
